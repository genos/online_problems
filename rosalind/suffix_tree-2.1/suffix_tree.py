...
must not contain the special symbol $.'''
        if t in s:
            raise ValueError(
                    "The suffix tree string must not contain terminal symbol!")
...
class GeneralisedSuffixTree(SuffixTree):
    ...
    def __init__(self, sequences):
        '''Build a generalised suffix tree.  The strings must not contain the
        special symbols $ or ascii numbers from SPECIAL + 1 to SPECIAL + the
        number of sequences.'''
        ...
        for i in xrange(len(sequences)):
            if chr(SPECIAL + i + 1) in sequences[i]:
                raise ValueError("The string can't contain chr({0})!".format(
                    SPECIAL + i + 1))
            concatString += sequences[i] + chr(SPECIAL + i + 1)
    ...
    def _translateIndex(self, idx):
        ...
        raise IndexError("Index out of range: {0}".format(idx))
