...
textToRow :: Text -> Row
textToRow = U.concatMap f . U.fromList . T.unpack
 where
  f :: Char -> Row
  f '.' = U.singleton 1
  f '^' = U.singleton 0
...
nextRow :: Row -> Row
nextRow r = U.zipWith3 nextTile r' (U.tail r') (U.tail $ U.tail r')
  where r' = 1 `U.cons` r `U.snoc` 1

test1 :: Bool
test1 = computed == expected
 where
  expected = V.map textToRow . V.fromList . T.lines $ test1Input
  computed = V.iterateN 3 nextRow $ V.head expected

test2 :: Bool
test2 = computed == expected
 where
