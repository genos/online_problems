...
parseHyper =
  Hyper . BC.pack <$> (AC.char '[' *> AC.many1' AC.letter_ascii <* AC.char ']')
...
abbas = concatMap mkABBA . subsLen 4
 where
...
part1 :: Int
part1 = either error id $ fmap countTLS ips
  where countTLS = length . filter tls

invert :: ABA -> ABA
invert (ABA a b) = ABA b a

abas :: ByteString -> [ABA]
abas = concatMap mkABA . subsLen 3
 where
...
part2 :: Int
part2 = either error id $ fmap countSSL ips
  where countSSL = length . filter ssl

main :: IO ()
main = do
  traverse_ print [test1, test2]
